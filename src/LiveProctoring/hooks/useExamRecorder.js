...
export function useExamRecorder(testid, uid, cameraStream, screenStream, isActive) {
  ...
  const camPromises = useRef([]);
  const screenPromises = useRef([]);
  ...
  const uploadChunk = useCallback(async (blob, blobName, blockId, blockIds, isLast) => {
    if (!isLast && (!blob || blob.size === 0)) return null;
    try {
      ...
      try {
        ...
        camRecorder.ondataavailable = async (e) => {
          if (e.data && e.data.size > 0) {
            ...
            camPromises.current.push(uploadChunk(e.data, camBlobName.current, blockId, camBlockIds.current, false));
          }
        };

        camRecorder.onstop = async () => {
          await Promise.all(camPromises.current);
          const url = await uploadChunk(null, camBlobName.current, null, camBlockIds.current, true);
          if (url) {
      ...
      try {
        ...
        screenRecorder.ondataavailable = async (e) => {
          if (e.data && e.data.size > 0) {
            ...
            screenPromises.current.push(uploadChunk(e.data, screenBlobName.current, blockId, screenBlockIds.current, false));
          }
        };

        screenRecorder.onstop = async () => {
          await Promise.all(screenPromises.current);
          const url = await uploadChunk(null, screenBlobName.current, null, screenBlockIds.current, true);
          if (url) {
