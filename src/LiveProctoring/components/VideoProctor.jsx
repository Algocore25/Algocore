...
import { usePersonDetection } from '../hooks/usePersonDetection.js';
...
export const VideoProctor = () => {
  ...
  const { detectObjects, isLoading, error, modelReady } = usePersonDetection();
  ...
  const [status, setStatus] = useState({
    personCount: 0,
    phoneCount: 0,
    violations: { multiplePerson: 0, noPerson: 0, mobileUsage: 0, total: 0 },
  ...
  const addAlert = useCallback((type, message) => {
    ...
    setAlerts((prev) => {
      // Prevent duplicate identical alerts in short timeframe
      const lastAlert = prev[prev.length - 1];
      if (lastAlert && lastAlert.message === message && 
          (new Date() - lastAlert.timestamp) < 3000) {
        return prev;
      }
  ...
  const runDetection = useCallback(async () => {
    ...
    setStatus((prev) => {
      ...
      if (personCount === 0 && prev.personCount !== 0) {
        ...
        alertAdded = true;
      } else if (personCount === 1 && prev.personCount !== 1 && !alertAdded && phoneCount === 0) {
        addAlert('success', 'Single person detected - compliant');
      }

      // Mobile detection logic
      if (phoneCount > 0 && prev.phoneCount === 0) {
        newViolations.mobileUsage++;
        newViolations.total++;
        addAlert('error', 'Mobile phone usage detected!');
      }
  ...
  }, [detectObjects, isProctoringActive, addAlert]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-slate-800 mb-2">Video Proctoring System</h1>
          <p className="text-slate-600">Advanced person detection for secure online examinations</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-lg shadow-lg p-6">
              ...
              <div className="relative bg-slate-900 rounded-lg overflow-hidden aspect-video">
                ...
                <VideoCanvas videoRef={videoRef} detections={detections} isActive={isProctoringActive} />
          ...
          <div className="space-y-6">
            ...
            <AlertPanel alerts={alerts} personCount={status.personCount} phoneCount={status.phoneCount} />
          </div>
        </div>
      </div>
    </div>
